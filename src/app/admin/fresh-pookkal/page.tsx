...
export default function FreshPookkalPage() {
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    fd.append("file", file);
    fd.append("folder", "uploads/fresh-pookkal");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-stone-200 shadow-sm">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-emerald-800 font-bold mb-1">
            <Flower2 className="w-4 h-4 text-emerald-500" /> Onam Fresh Pookkal
          </div>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-stone-900">
            Fresh Flowers & Garlands ({list.length})
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setShowForm(true); setEditing(null); setForm({ name: "", description: "", price: "", compareAtPrice: "", images: [], sizes: [...lastSizes], colorVariants: [{ color: "", image: "", isDefault: true }], sizePrices: { ...lastSizePrices }, orderType: "add_to_bag", stock: 50, sortOrder: 0, isActive: true, hideOrdering: false }); }} className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl transition font-bold text-xs shadow-md">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-stone-200 p-6 md:p-8 shadow-xl space-y-5 animate-slide-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. White Rose Garland" className="w-full px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl focus:bg-white outline-none transition text-sm font-medium" />
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-stone-600 mb-1.5">Base Price (£) *</label>
              <input required type="number" step="0.01" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="e.g. 12.50" className="w-full px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl focus:bg-white outline-none transition text-sm font-bold" />
            </div>
            <div>
          ...
          <div className="bg-emerald-50/50 border border-emerald-200 p-5 rounded-2xl space-y-3">
            <h3 className="font-display text-base font-bold text-emerald-950 flex items-center gap-2">
              🌿 Gram Sizes {form.sizes.length > 0 && <span className="text-xs font-normal text-emerald-600">({form.sizes.length} selected)</span>}
            </h3>
            {form.sizes.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {form.sizes.map((size, idx) => (
                  <div key={idx} className="inline-flex items-center gap-1 bg-emerald-100 border border-emerald-400 text-emerald-800 text-xs font-bold px-3 py-1.5 rounded-lg">
                    ✓ {size}
                    <input type="number" step="0.01" placeholder="Price" value={form.sizePrices[size] || ""} onChange={(e) => updateSizePrice(size, e.target.value)} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()} className="w-16 px-1 py-0.5 bg-white border border-emerald-300 rounded text-center text-[10px] font-bold outline-none" />
                    <span className="text-[9px] text-emerald-500 font-mono">£</span>
                    <button type="button" onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); toggleSize(size); }} className="ml-0.5 text-emerald-500 hover:text-red-500 font-bold">×</button>
                  </div>
                ))}
              </div>
            )}
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-1">
                {gramPresets.map((sz) => (
                  <button key={sz} type="button" onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); toggleSize(sz); }} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition border ${form.sizes.includes(sz) ? "bg-emerald-100 border-emerald-400 text-emerald-800" : "bg-white border-stone-200 text-stone-600 hover:bg-emerald-50 hover:border-emerald-300"}`}>
          ...
          <div className="border border-amber-200 bg-amber-50/50 p-5 rounded-2xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-amber-800 mt-0.5">Add color options (White Rose, Pink, Red, etc.) with dedicated variant photos.</p>
              </div>
            ...
            <div className="bg-white/80 p-3 rounded-xl border border-amber-200/80 space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {[
                  { name: "White Rose", bg: "#fefce8", text: "#713f12" },
                  { name: "Pink Rose", bg: "#fbcfe8", text: "#831843" },
                  { name: "Red Rose", bg: "#dc2626", text: "#ffffff" },
                  { name: "Yellow Marigold", bg: "#f59e0b", text: "#451a03" },
            ...
            {form.colorVariants.map((cv, idx) => (
              <div key={idx} className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-3 bg-white border border-amber-200 rounded-xl shadow-xs">
                <div className="flex-1 w-full sm:w-auto">
                  ...
                  <input placeholder="e.g. Pink Rose" value={cv.color} onChange={(e) => updateColorVariantName(idx, e.target.value)} className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm outline-none font-medium" />
                </div>
        ...
        <table className="w-full text-left border-collapse text-xs">
          ...
          <tbody className="divide-y divide-stone-150">
            {filteredList.map((d) => {
              ...
              return (
                <tr key={d.id} className={`hover:bg-stone-50/70 transition ${isSelected ? "bg-amber-50/40" : ""}`}>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      {d.images?.[0] ? <img src={d.images[0]} alt="" className="w-12 h-12 object-cover rounded-xl border border-stone-200 shrink-0" /> : <div className="w-12 h-12 bg-stone-100 rounded-xl flex items-center justify-center text-stone-400 shrink-0"><Flower2 className="w-5 h-5 text-emerald-500" /></div>}
                      <div>
            ...
            {filteredList.length === 0 && (
              <tr><td colSpan={6} className="p-12 text-center text-stone-500">No Fresh Pookkal items found. Click "+ Add Fresh Pookkal" above to create one.</td></tr>
            )}
          </tbody>
        </table>
      </div>
