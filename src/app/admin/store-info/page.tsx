...
import { Save, Store, MapPin, Phone, Mail, MessageCircle, Info, Megaphone, Truck, Gift, DollarSign } from "lucide-react";
...
export default function StoreInfoPage() {
  ...
  async function fetchSettings() {
    ...
    if (!map.free_delivery_threshold) map.free_delivery_threshold = "35";
    if (!map.delivery_fee) map.delivery_fee = "5.50";
    if (!map.delivery_time) map.delivery_time = "2-4 business days";
  ...
  async function saveAllSettings() {
    const keys = [
      ...
      "free_delivery_threshold", "delivery_fee", "delivery_time"
    ];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm lg:col-span-2">
          <h2 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2 border-b pb-3">
            <Truck className="w-5 h-5 text-emerald-600" /> Delivery Settings
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Gift className="w-4 h-4 text-emerald-500"/> Free Delivery Threshold (£)
              </label>
              <input 
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" 
              />
              <p className="text-xs text-slate-500 mt-1">Orders over this amount get FREE delivery. Leave blank to disable.</p>
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <DollarSign className="w-4 h-4 text-slate-500"/> Standard Delivery Fee (£)
              </label>
              <input 
                type="number" step="0.01"
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" 
              />
              <p className="text-xs text-slate-500 mt-1">Delivery fee for orders below free threshold.</p>
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Truck className="w-4 h-4 text-slate-500"/> Delivery Time
              </label>
              <input 
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" 
              />
              <p className="text-xs text-slate-500 mt-1">Estimated delivery time shown to customers.</p>
            </div>
          </div>
        </div>
