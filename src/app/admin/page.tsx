...
export default function AdminDashboard() {
  const [stats, setStats] = useState({
    ...
    orders: 0, revenue: 0, offers: 0, dresses: 0, winners: 0, pookkalam: 0, sadhya: 0,
  });
  ...
  async function fetchStats() {
    try {
      const [collections, categories, items, slides, orders, offers, dresses, winners, pookkalam, sadhya] = await Promise.all([
        ...
        fetch("/api/items?categoryId=15").then((r) => r.json()),
      ]);
      ...
      setStats({
        ...
        pookkalam: pookkalam.length || 0, sadhya: sadhya.length || 0,
      });
    } catch (e) {}
    setLoading(false);
  }
