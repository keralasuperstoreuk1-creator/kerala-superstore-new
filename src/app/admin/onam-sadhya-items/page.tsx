...
import {
  ...
  Palette, Search, Salad
} from "lucide-react";

export default function OnamSadhyaItemsPage() {
  ...
  const [catId, setCatId] = useState<number>(15);
  ...
  async function fetchSettings() {
    try {
      ...
      if (data) {
        const v = data.find((s: any) => s.key === "onam_sadhya_category_id");
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    fd.append("file", file);
    fd.append("folder", "uploads/onam-sadhya");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-stone-200 shadow-sm">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-amber-800 font-bold mb-1">
            <Salad className="w-4 h-4 text-amber-500" /> Onam Sadhya Items
          </div>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-stone-900">
            Sadhya Feast Items ({list.length})
          </h1>
          <p className="text-stone-600 text-sm mt-1">
            സാധ്യ ഭോജനത്തിന്റെ സാധനങ്ങൾ — ബാനാണ, സാമ്പാർ, പായസം, അവിയൽ എന്നിവ ഗ്രാമിനും കളറിനും അനുസരിച്ച് ചേർക്കുക।
          </p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-stone-200 p-6 md:p-8 shadow-xl space-y-5 animate-slide-up">
          <div className="flex items-center justify-between border-b border-stone-100 pb-4">
            <h2 className="font-display text-xl font-bold text-stone-900">{editing ? `Edit: ${editing.name}` : "Add New Sadhya Item"}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Banana Chips" className="w-full px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl focus:bg-white outline-none transition text-sm font-medium" />
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-stone-600 mb-1.5">Base Price (£) *</label>
              <input required type="number" step="0.01" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="e.g. 5.00" className="w-full px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl focus:bg-white outline-none transition text-sm font-bold" />
            </div>
            <div>
          ...
          <div className="bg-amber-50/50 border border-amber-200 p-5 rounded-2xl space-y-3">
            <h3 className="font-display text-base font-bold text-amber-950 flex items-center gap-2">
              🌿 Gram Sizes {form.sizes.length > 0 && <span className="text-xs font-normal text-amber-600">({form.sizes.length} selected)</span>}
            </h3>
            {form.sizes.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {form.sizes.map((size, idx) => (
                  <div key={idx} className="inline-flex items-center gap-1 bg-amber-100 border border-amber-400 text-amber-800 text-xs font-bold px-3 py-1.5 rounded-lg">
                    ✓ {size}
                    <input type="number" step="0.01" placeholder="Price" value={form.sizePrices[size] || ""} onChange={(e) => updateSizePrice(size, e.target.value)} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()} className="w-16 px-1 py-0.5 bg-white border border-amber-300 rounded text-center text-[10px] font-bold outline-none" />
                    <span className="text-[9px] text-amber-500 font-mono">£</span>
                    <button type="button" onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); toggleSize(size); }} className="ml-0.5 text-amber-500 hover:text-red-500 font-bold">×</button>
                  </div>
                ))}
              </div>
            )}
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-1">
                {gramPresets.map((sz) => (
                  <button key={sz} type="button" onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); toggleSize(sz); }} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition border ${form.sizes.includes(sz) ? "bg-amber-100 border-amber-400 text-amber-800" : "bg-white border-stone-200 text-stone-600 hover:bg-amber-50 hover:border-amber-300"}`}>
            ...
            <div className="flex gap-2">
              <input id="sadhyaCustomSize" placeholder="Custom gram (e.g. 150g, 1.5kg)" className="flex-1 px-3 py-2 bg-white border border-amber-200 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-amber-400" onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); const val = (e.target as HTMLInputElement).value.trim(); if (val && !form.sizes.includes(val)) { toggleSize(val); (e.target as HTMLInputElement).value = ""; } } }} />
              <button type="button" onClick={() => { const inp = document.getElementById("sadhyaCustomSize") as HTMLInputElement; const val = inp?.value?.trim(); if (val && !form.sizes.includes(val)) { toggleSize(val); inp.value = ""; } }} className="px-4 py-2 bg-amber-600 text-white text-xs font-bold rounded-lg hover:bg-amber-700 transition">Add</button>
            </div>
            <p className="text-[10px] text-amber-600 italic">💡 Each size × each color = one variant on the website.</p>
          </div>
          ...
          <div className="border border-amber-200 bg-amber-50/50 p-5 rounded-2xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-amber-800 mt-0.5">Add color options with dedicated variant photos.</p>
              </div>
        ...
        <table className="w-full text-left border-collapse text-xs">
          ...
          <tbody className="divide-y divide-stone-150">
            {filteredList.map((d) => {
              ...
              return (
                <tr key={d.id} className={`hover:bg-stone-50/70 transition ${isSelected ? "bg-amber-50/40" : ""}`}>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      {d.images?.[0] ? <img src={d.images[0]} alt="" className="w-12 h-12 object-cover rounded-xl border border-stone-200 shrink-0" /> : <div className="w-12 h-12 bg-stone-100 rounded-xl flex items-center justify-center text-stone-400 shrink-0"><Salad className="w-5 h-5 text-amber-500" /></div>}
                      <div>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1 flex-wrap">
                      {sizes.length > 0 ? sizes.map((s: string, i: number) => (<span key={i} className="px-1.5 py-0.5 rounded bg-amber-50 border border-amber-200 text-[10px] font-semibold text-amber-800">{s}</span>)) : <span className="text-stone-400 text-[10px] italic">—</span>}
                    </div>
                  </td>
                  <td className="p-4">
                    <div className="flex items-center gap-1 flex-wrap">
            ...
            {filteredList.length === 0 && (
              <tr><td colSpan={6} className="p-12 text-center text-stone-500">No Sadhya items found. Click "+ Add Sadhya Item" above to create one.</td></tr>
            )}
          </tbody>
        </table>
      </div>
