...
import { Store, Bell, Package, TrendingUp, CheckCircle, Truck, Search, LogOut, Clock, Eye, X, ChevronLeft } from "lucide-react";
...
export default function AdminAppPage() {
  ...
  const logout = () => { localStorage.removeItem("admin_app_user"); setAdmin(null); setEmail(""); setPassword(""); };
  ...
  const filteredOrders = orders.filter((o) => {
    if (searchQuery) {
      ...
      return o.customerName.toLowerCase().includes(q) || o.orderNumber.toLowerCase().includes(q) || o.customerPhone.includes(q);
    }
    return true;
  });
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-stone-900 flex items-center justify-center p-4">
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            <div className="w-16 h-16 bg-white/10 backdrop-blur rounded-2xl flex items-center justify-center mx-auto mb-4"><Store className="w-8 h-8 text-emerald-300" /></div>
            <h1 className="text-2xl font-bold text-white">Admin App</h1>
            <p className="text-emerald-200/70 text-sm mt-1">Kerala Super Store</p>
          </div>
          <form onSubmit={login} className="bg-white/10 backdrop-blur-md rounded-3xl p-6 space-y-4 border border-white/10">
            <div><label className="block text-xs font-medium text-emerald-200 mb-1.5">Email</label><input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/40 text-sm outline-none focus:ring-2 focus:ring-emerald-400" placeholder="admin@keralasuperstore.com" /></div>
            <div><label className="block text-xs font-medium text-emerald-200 mb-1.5">Password</label><input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/40 text-sm outline-none focus:ring-2 focus:ring-emerald-400" placeholder="••••••••" /></div>
    ...
    <div className="min-h-screen bg-stone-50 pb-24">
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-stone-200 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-stone-900 text-sm">Admin App</span>
        </div>
        <div className="flex items-center gap-3">
          {newOrdersCount > 0 && (
            <span className="relative">
              <Bell className="w-5 h-5 text-amber-600" />
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">{newOrdersCount}</span>
            </span>
          )}
              ...
              {Object.entries(stats.monthly).reverse().map(([month, data]) => (
                <div key={month} className="flex items-center justify-between py-2 border-b border-stone-100 last:border-0">
                  ...
                  <div className="text-right">
                    <span className="text-sm font-bold text-stone-900">{data.orders} orders</span>
                    <span className="text-xs text-stone-500 ml-2">£{data.revenue.toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {Object.entries(stats.daily).reverse().slice(0, 14).map(([day, data]) => (
                <div key={day} className="flex items-center justify-between py-2 border-b border-stone-100 last:border-0">
                  ...
                  <div className="text-right">
                    <span className="text-sm font-medium text-stone-900">{data.orders} orders</span>
                    <span className="text-xs text-stone-500 ml-2">£{data.revenue.toFixed(2)}</span>
                  </div>
                </div>
              ))}
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-2 pt-2">
                {selectedOrder.status === "pending" && (
                  <button onClick={() => updateStatus(selectedOrder.id, "confirmed")} disabled={updating} className="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm transition disabled:opacity-50 flex items-center justify-center gap-2"><CheckCircle className="w-4 h-4" /> Confirm Order</button>
                )}
                {selectedOrder.status === "confirmed" && (
                  <button onClick={() => updateStatus(selectedOrder.id, "processing")} disabled={updating} className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition disabled:opacity-50 flex items-center justify-center gap-2"><Clock className="w-4 h-4" /> Mark Processing</button>
                )}
                {selectedOrder.status === "processing" && (
                  <button onClick={() => updateStatus(selectedOrder.id, "delivered")} disabled={updating} className="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm transition disabled:opacity-50 flex items-center justify-center gap-2"><Truck className="w-4 h-4" /> Mark Delivered</button>
                )}
                {!["delivered", "cancelled"].includes(selectedOrder.status) && (
                  <button onClick={() => updateStatus(selectedOrder.id, "cancelled")} disabled={updating} className="w-full py-3 rounded-xl bg-red-100 hover:bg-red-200 text-red-700 font-bold text-sm transition disabled:opacity-50">Cancel Order</button>
                )}
              </div>
            </div>
          </div>
