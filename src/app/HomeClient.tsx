              ...
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 bg-slate-50 rounded-lg p-3">
                    ...
                    <div className="text-[9px] text-red-700 font-bold bg-red-100 border border-red-300 rounded px-1 whitespace-nowrap overflow-visible">ID={item.itemId} TYPE={item.itemType} IMG={item.item?.images?.[0]?.slice(0,30) || item.item?.image?.slice(0,30) || item.variant?.images?.[0]?.slice(0,30) || 'NONE'}</div>
